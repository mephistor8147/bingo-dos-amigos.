...
import { getRoomCurrentPrize } from "../utils";

interface AdminRoomsProps {
  rooms: Room[];
  onCreateRoom: (
    room: Partial<Room & { botsEnabled?: boolean; maxBots?: number }>,
  ) => void;
  ...
  autoRoomEntryFee?: number;
  autoRoomPrizeMode?: 'fixed' | 'cumulative' | 'cumulative_jackpot';
  autoRoomPrizeValue?: number;
  ...
  onUpdateAutoRoomEntryFee?: (val: number) => void;
  onUpdateAutoRoomPrizeMode?: (val: 'fixed' | 'cumulative' | 'cumulative_jackpot') => void;
  onUpdateAutoRoomPrizeValue?: (val: number) => void;
}
...
export function AdminRooms({
  ...
  onUpdateAutoRoomEntryFee,
  onUpdateAutoRoomPrizeMode,
  onUpdateAutoRoomPrizeValue,
}: AdminRoomsProps) {
  ...
  const [prizeMode, setPrizeMode] = useState<
    "fixed" | "cumulative" | "cumulative_jackpot" | "four_balls_double"
  >("fixed");
  ...
  const [gameMode, setGameMode] = useState<GameMode>("full_card");

  const [fourBallsStage1, setFourBallsStage1] = useState<number>(30);
  const [fourBallsStage2, setFourBallsStage2] = useState<number>(42);
  const [fourBallsStage3, setFourBallsStage3] = useState<number>(50);
  ...
  const handleCreate = (e: React.FormEvent) => {
    ...
    onCreateRoom({
      name: newName,
      entryFee: fee,
      prize: prizeMode === "cumulative" ? 0 : prize,
      prizeMode,
      ...
      maxPlayers: 10,
      gameMode:
        prizeMode === "four_balls_double" ? "four_balls_double" : gameMode,
      botsEnabled,
      maxBots,
      theme,
      fourBallsStage1Limit:
        prizeMode === "four_balls_double" || gameMode === "four_balls_double"
          ? fourBallsStage1
          : undefined,
      fourBallsStage2Limit:
        prizeMode === "four_balls_double" || gameMode === "four_balls_double"
          ? fourBallsStage2
          : undefined,
      fourBallsStage3Limit:
        prizeMode === "four_balls_double" || gameMode === "four_balls_double"
          ? fourBallsStage3
          : undefined,
    });
    ...
    setMaxBots(2);
    setPrizeMode("fixed");
    setGameMode("full_card");
    setFourBallsStage1(30);
    setFourBallsStage2(42);
    setFourBallsStage3(50);
    setShowCreate(false);
  };

  // Real-time Game Economics and Stats Calculations
  const totalArrecadado = rooms.reduce((acc, r) => {
    const playersCount = r.players?.length || 0;
    return acc + r.entryFee * playersCount;
  }, 0);

  const totalPremios = rooms.reduce((acc, r) => {
    if (r.status === "finished") {
      return acc + (getRoomCurrentPrize(r) || 0);
    }
    return acc;
  }, 0);

  const lucroCasa = totalArrecadado - totalPremios;

  let totalHumans = 0;
  let totalBots = 0;
  rooms.forEach((r) => {
    (r.players || []).forEach((p) => {
      if (p.id.startsWith("bot") || p.id.includes("bot")) {
        totalBots++;
      } else {
        totalHumans++;
      }
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium text-sm">
            Monitore a economia do jogo e configure rodadas ao vivo
          </p>
        </div>
        <button
          onClick={() => setShowCreate(!showCreate)}
          className="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-3 rounded-xl font-bold flex items-center gap-2 shadow-lg shadow-indigo-600/20 active:scale-95 transition-all text-sm w-full md:w-auto justify-center cursor-pointer"
        >
      ...
      {/* Bento Grid Analytics & Finance System Overview */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {/* Entry Fees collected */}
        <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800/80 p-4 rounded-3xl shadow-sm flex flex-col justify-between">
          ...
          <div>
            <div className="text-xl md:text-2xl font-black text-slate-800 dark:text-white">
              {totalArrecadado.toLocaleString()}
            </div>
            <p className="text-[10px] text-slate-400 font-bold">
              Moedas de taxas de entrada
            </p>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800/80 p-4 rounded-3xl shadow-sm flex flex-col justify-between">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 font-bold">
              Moedas pagas a campeões
            </p>
          </div>
        </div>

        {/* House Profit Ledger Net */}
        <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800/80 p-4 rounded-3xl shadow-sm flex flex-col justify-between">
          <div className="flex justify-between items-start mb-2">
            <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest leading-none">
              Giro Líquido
            ...
            <span
              className={`text-xs font-black px-1.5 py-0.5 rounded-md ${lucroCasa >= 0 ? "bg-emerald-50 text-emerald-600 dark:bg-emerald-950/30" : "bg-red-50 text-red-650"}`}
            >
              {lucroCasa >= 0 ? "+LUCRO" : "-DEFICIT"}
            </span>
          </div>
          <div>
            <div
              className={`text-xl md:text-2xl font-black ${lucroCasa >= 0 ? "text-emerald-500" : "text-amber-500"}`}
            >
              {lucroCasa.toLocaleString()}
            </div>
            <p className="text-[10px] text-slate-400 font-bold">
              Saldo corrente da plataforma
            </p>
          </div>
        </div>

        {/* Users distribution Humans vs Bots */}
        <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800/80 p-4 rounded-3xl shadow-sm flex flex-col justify-between">
          <div className="flex justify-between items-start mb-2">
            <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest leading-none">
              Participações
      ...
      <div className="bg-slate-100 dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800 rounded-3xl p-6 mb-8 space-y-4">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <h3 className="font-extrabold text-slate-800 dark:text-white text-sm md:text-base">
                Criação Automática de Salas
              </h3>
              <p className="text-slate-500 dark:text-slate-400 text-xs font-semibold">
                Cria novas salas agendadas contendo 2 bots participantes por
                padrão.
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4 border-t border-slate-200/60 dark:border-slate-800/80">
          ...
          <div className="bg-white dark:bg-slate-950 p-4 rounded-2xl border border-slate-200/50 dark:border-slate-800 flex flex-col justify-between gap-1.5 shadow-sm">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
              Início do Funcionamento (Diário)
            </span>
            <input
              type="time"
              value={autoRoomStartHour}
              onChange={(e) =>
                onUpdateAutoRoomHours(e.target.value, autoRoomEndHour)
              }
              disabled={!autoRoomEnabled}
          ...
          <div className="bg-white dark:bg-slate-950 p-4 rounded-2xl border border-slate-200/50 dark:border-slate-800 flex flex-col justify-between gap-1.5 shadow-sm">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
              Fim do Funcionamento (Diário)
            </span>
            <input
              type="time"
              value={autoRoomEndHour}
              onChange={(e) =>
                onUpdateAutoRoomHours(autoRoomStartHour, e.target.value)
              }
              disabled={!autoRoomEnabled}
              className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-2.5 py-2 text-xs font-black text-slate-700 dark:text-slate-300 outline-none disabled:opacity-40 cursor-text"
            />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4 border-t border-slate-200/60 dark:border-slate-800/80">
          ...
          <div className="bg-white dark:bg-slate-950 p-4 rounded-2xl border border-slate-200/50 dark:border-slate-800 flex flex-col justify-between gap-1.5 shadow-sm">
            ...
            <input
              ...
              onChange={(e) =>
                onUpdateAutoRoomBaseName &&
                onUpdateAutoRoomBaseName(e.target.value)
              }
              disabled={!autoRoomEnabled}
          ...
          <div className="bg-white dark:bg-slate-950 p-4 rounded-2xl border border-slate-200/50 dark:border-slate-800 flex flex-col justify-between gap-1.5 shadow-sm">
            ...
            <input
              ...
              onChange={(e) =>
                onUpdateAutoRoomBotsCount &&
                onUpdateAutoRoomBotsCount(Number(e.target.value))
              }
              disabled={!autoRoomEnabled}
          ...
          <div className="bg-white dark:bg-slate-950 p-4 rounded-2xl border border-slate-200/50 dark:border-slate-800 flex flex-col justify-between gap-1.5 shadow-sm font-semibold">
            ...
            <div className="flex gap-2 w-full">
              <input
                ...
                onChange={(e) =>
                  onUpdateAutoRoomSequenceNumber &&
                  onUpdateAutoRoomSequenceNumber(Number(e.target.value))
                }
                disabled={!autoRoomEnabled}
                className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-2.5 py-2 text-xs font-black text-slate-700 dark:text-slate-300 outline-none disabled:opacity-40"
              />
              <button
                ...
                onClick={() =>
                  onUpdateAutoRoomSequenceNumber &&
                  onUpdateAutoRoomSequenceNumber(1)
                }
                disabled={!autoRoomEnabled}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-slate-200/60 dark:border-slate-800/80">
          ...
          <div className="bg-white dark:bg-slate-950 p-4 rounded-2xl border border-slate-200/50 dark:border-slate-800 flex flex-col justify-between gap-1.5 shadow-sm">
            ...
            <select
              ...
              onChange={(e) =>
                onUpdateAutoRoomGameMode &&
                onUpdateAutoRoomGameMode(e.target.value as GameMode)
              }
              disabled={!autoRoomEnabled}
              className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-2.5 py-2 text-xs font-black text-slate-700 dark:text-slate-300 outline-none disabled:opacity-40"
            >
              ...
              <option value="four_balls_double">4 Bolas ou Dobra (Especial)</option>
          ...
          <div className="bg-white dark:bg-slate-950 p-4 rounded-2xl border border-slate-200/50 dark:border-slate-800 flex flex-col justify-between gap-1.5 shadow-sm">
            ...
            <input
              ...
              onChange={(e) =>
                onUpdateAutoRoomQuantity &&
                onUpdateAutoRoomQuantity(Number(e.target.value))
              }
              disabled={!autoRoomEnabled}
              className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-2.5 py-2 text-xs font-black text-slate-700 dark:text-slate-300 outline-none disabled:opacity-40"
            />
        ...
        {/* 💰 Configurações de Custo e Prêmio para Salas Automáticas */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4 border-t border-slate-200/60 dark:border-slate-800/80">
          {/* Valor da Cartela */}
          <div className="bg-white dark:bg-slate-950 p-4 rounded-2xl border border-slate-200/50 dark:border-slate-800 flex flex-col justify-between gap-1.5 shadow-sm">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
              1° Custo de Entrada da Cartela (Moedas)
            </span>
            <input
              type="number"
              min="1"
              value={autoRoomEntryFee ?? 10}
              onChange={(e) =>
                onUpdateAutoRoomEntryFee &&
                onUpdateAutoRoomEntryFee(Number(e.target.value))
              }
              disabled={!autoRoomEnabled}
              className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-2.5 py-2 text-xs font-black text-slate-700 dark:text-slate-300 outline-none disabled:opacity-40"
            />
          ...
          <div className="bg-white dark:bg-slate-950 p-4 rounded-2xl border border-slate-200/50 dark:border-slate-800 flex flex-col justify-between gap-1.5 shadow-sm">
            ...
            <select
              value={autoRoomPrizeMode ?? "fixed"}
              onChange={(e) =>
                onUpdateAutoRoomPrizeMode &&
                onUpdateAutoRoomPrizeMode(e.target.value as any)
              }
              disabled={!autoRoomEnabled}
              className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-2.5 py-2 text-xs font-black text-slate-700 dark:text-slate-300 outline-none disabled:opacity-40"
            >
              <option value="fixed">Tipo 1: Prêmio Fixo Pré-definido</option>
              <option value="cumulative">Tipo 2: Soma do Custo dos Participantes</option>
              <option value="cumulative_jackpot">Tipo 3: Acumulado com Taxa (20% recolhido)</option>
            </select>
          </div>
          ...
          <div className="bg-white dark:bg-slate-950 p-4 rounded-2xl border border-slate-200/50 dark:border-slate-800 flex flex-col justify-between gap-1.5 shadow-sm">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
              {autoRoomPrizeMode === "fixed" ? "Prêmio Fixo Configurado" : "Descrição do Prêmio"}
            </span>
            {autoRoomPrizeMode === "fixed" ? (
              <input
                type="number"
                min="1"
                value={autoRoomPrizeValue ?? 100}
                onChange={(e) =>
                  onUpdateAutoRoomPrizeValue &&
                  onUpdateAutoRoomPrizeValue(Number(e.target.value))
                }
                disabled={!autoRoomEnabled}
                className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-2.5 py-2 text-xs font-black text-slate-700 dark:text-slate-300 outline-none disabled:opacity-40"
              />
            ) : (
              <div className="text-[11px] font-bold text-indigo-600 dark:text-indigo-400 py-2">
                {autoRoomPrizeMode === "cumulative" 
                  ? "Calculado dinamicamente com base nas cartelas compradas." 
                  : "80% da soma das cartelas vai para o vencedor. 20% é recolhido."}
              </div>
            )}
          </div>
        </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-left">
            <div>
              <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest block">
                Rádio Online Automática (Streaming)
              </span>
              <span className="text-xs text-slate-500 dark:text-slate-400 font-semibold">
                Toca por padrão nas salas criadas automaticamente.
              </span>
            </div>
            <select
            ...
            >
              ...
              <option value="https://live.hunter.fm/sertanejo_stream?ag=mp3">
                Sertanejo
              </option>
              <option value="https://live.hunter.fm/pop_stream?ag=mp3">
                Pop
              </option>
              <option value="https://live.hunter.fm/pagode_stream?ag=mp3">
                Pagode
              </option>
              <option value="https://live.hunter.fm/rock_stream?ag=mp3">
                Rook
              </option>
              <option value="https://live.hunter.fm/master_stream?ag=mp3">
                Master
              </option>
              <option value="https://live.hunter.fm/mpb_stream?ag=mp3">
                Mpb
              </option>
              <option value="https://live.hunter.fm/gospel_stream?ag=mp3">
                Gostei
              </option>
              <option value="https://live.hunter.fm/hitsbrasil_stream?ag=mp3">
                Hits Brasil
              </option>
              <option value="https://live.hunter.fm/pop2k_stream?ag=mp3">
                Pop 2k
              </option>
              <option value="https://live.hunter.fm/modasertaneja_stream?ag=mp3">
                Moda sertaneja
              </option>
              <option value="https://live.hunter.fm/80s_stream?ag=mp3">
                80s
              </option>
              <option value="https://live.hunter.fm/kpop_stream?ag=mp3">
                Kpop
              </option>
              <option value="https://live.hunter.fm/pisadinha_stream?ag=mp3">
                Pisadinha
              </option>
            </select>
          </div>
      ...
      {showCreate && (
        <form
          ...
          className="bg-white dark:bg-slate-900 p-4 sm:p-6 rounded-3xl shadow-xl border border-slate-100 dark:border-slate-800 mb-8 space-y-4"
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-3 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none font-bold text-indigo-600 dark:text-indigo-400"
              />
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">
            ...
            <div>
              ...
              <input
                ...
                className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-3 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none font-bold text-amber-600 dark:text-amber-400"
              />
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setPrizeMode(mode);
                  if (mode === "four_balls_double") {
                    setGameMode("four_balls_double");
                  }
                }}
                className="w-full bg-slate-50 dark:bg-slate-800 text-slate-800 dark:text-slate-100 border border-slate-200 dark:border-slate-700 p-3 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none font-bold text-xs"
              >
                <option value="fixed">Modo 1: Definida pelo Administrador</option>
                <option value="cumulative">Modo 2: Acumulado por Participantes (100% pago)</option>
                <option value="cumulative_jackpot">Modo 3: Acumulado com Taxa (80% pago / 20% recolhido)</option>
                <option value="four_balls_double">Modo 4: 4 Bolas ou Dobra (Modo Próximos/Dobra)</option>
              </select>
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">
                {prizeMode === "fixed"
                  ? "Premiação (Moedas)"
                  : prizeMode === "cumulative" || prizeMode === "cumulative_jackpot"
                    ? "Premiação Estimada"
                    : "Premiação Mínima Inicial"}
              </label>
              <input
                type="number"
                min="0"
                value={prizeMode === "cumulative" ? fee * 5 : prizeMode === "cumulative_jackpot" ? Math.floor(fee * 5 * 0.8) : prize}
                onChange={(e) => setPrize(Number(e.target.value))}
                disabled={prizeMode === "cumulative" || prizeMode === "cumulative_jackpot"}
                className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-3 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none font-bold text-emerald-600 dark:text-emerald-400 disabled:opacity-50"
              />
              {(prizeMode === "cumulative" || prizeMode === "cumulative_jackpot") && (
                <span className="text-[10px] text-slate-400 font-bold mt-1 block">
                  {prizeMode === "cumulative"
                    ? "Calculado dinamicamente: Entrada × Total de Jogadores adicionados."
                    : "Calculado dinamicamente: Entrada × Total de Jogadores adicionados × 80% (20% taxa recolhida para o acumulado)."}
                </span>
              )}
              {prizeMode === "four_balls_double" && (
                <span className="text-[10px] text-slate-400 font-bold mt-1 block font-semibold text-emerald-600 dark:text-emerald-400">
                  Prêmio inicial que se acumula e sobe a cada "dobra" aceita!
                </span>
              )}
            </div>

            {(prizeMode === "four_balls_double" || gameMode === "four_balls_double") && (
              <div className="sm:col-span-2 bg-indigo-50/50 dark:bg-indigo-950/20 p-4 rounded-2xl border border-indigo-150 dark:border-indigo-900/40 space-y-3">
                <h4 className="font-extrabold text-xs text-indigo-700 dark:text-indigo-400 uppercase tracking-wider flex items-center gap-2">
                  <span className="w-2 h-2 bg-indigo-500 rounded-full animate-ping"></span>
                  Configurações do Modo 4 Bolas ou Dobra
                </h4>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div>
                    <label className="block text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">
                      Bolas Estágio 1 (Prompt 1ª Dobra)
                    </label>
                    <input
                      type="number"
                      min="5"
                      max="75"
                      value={fourBallsStage1}
                      onChange={(e) => setFourBallsStage1(Math.max(5, Math.min(75, Number(e.target.value))))}
                      className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-2.5 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none font-bold text-xs"
                    />
                    <span className="text-[9px] text-slate-400">Sorteio inicia e para com este total. (Padrão: 30)</span>
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">
                      Bolas Estágio 2 (Prompt 2ª Dobra)
                    </label>
                    <input
                      type="number"
                      min={fourBallsStage1 + 1}
                      max="75"
                      value={fourBallsStage2}
                      onChange={(e) => setFourBallsStage2(Math.max(fourBallsStage1 + 1, Math.min(75, Number(e.target.value))))}
                      className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-2.5 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none font-bold text-xs"
                    />
                    <span className="text-[9px] text-slate-400">Limite de bolas após aceitar a 1ª dobra. (Padrão: 42)</span>
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">
                      Bolas Estágio 3 (Fim do Jogo)
                    </label>
                    <input
                      type="number"
                      min={fourBallsStage2 + 1}
                      max="75"
                      value={fourBallsStage3}
                      onChange={(e) => setFourBallsStage3(Math.max(fourBallsStage2 + 1, Math.min(75, Number(e.target.value))))}
                      className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-2.5 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none font-bold text-xs"
                    />
                    <span className="text-[9px] text-slate-400">Limite absoluto com a 2ª dobra final. (Padrão: 50)</span>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="sm:col-span-2">
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                <input
                  ...
                  className="flex-1 min-w-0 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-3 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none font-bold text-slate-800 dark:text-slate-100"
                />
            ...
            <div className="sm:col-span-2">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-1">
                <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">
                  Link de Rádio Online (Streaming)
                </label>
                <select
              ...
              <div className="space-y-2 mb-4">
                ...
                >
                  ...
                  <option value="custom">
                    -- Utilizar URL Personalizada ou Arquivo --
                  </option>
                </select>
                ...
                <div className="space-y-2 bg-slate-50 dark:bg-slate-800/50 p-3 rounded-2xl border border-slate-100 dark:border-slate-800">
                  ...
                  {roomBgImage && (
                    <div className="flex items-center gap-2 pt-1 border-t border-slate-200/40 dark:border-slate-700 mt-2">
                      <div
                        className="w-20 h-10 rounded bg-cover bg-center border border-slate-200 dark:border-slate-700"
              ...
              >
                <option value="full_card">Cartela Cheia</option>
                <option value="line">Linha (Vertical/Horizontal)</option>
                <option value="block_of_4">
                  4 Números Próximos (Bloco 2x2)
                </option>
                <option value="four_balls_double">4 Bolas ou Dobra (Especial)</option>
              ...
              <div className="border-t border-slate-100 dark:border-slate-800 pt-4 mt-2">
                <h4 className="font-extrabold text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">
                  🤖 Bots na Sala (Até 5)
                </h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <button
            ...
            className="w-full bg-slate-800 hover:bg-slate-900 dark:bg-indigo-600 dark:hover:bg-indigo-700 text-white p-4 rounded-xl font-bold transition-colors disabled:opacity-50 mt-4 active:scale-[0.98]"
          >
            Criar e Agendar
          </button>
        </form>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {rooms.length === 0 ? (
          <div className="col-span-full py-16 text-center text-slate-400 font-medium bg-white/50 dark:bg-slate-900/40 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-3xl">
            Nenhuma sala criada.
          </div>
        ) : (
          rooms.map((room) => (
              ...
              <div className="flex justify-between items-start mb-4 relative z-10">
                <div>
                  <h4 className="font-black text-xl text-slate-800 dark:text-white flex items-center gap-2.5">
                    <span className="text-2xl bg-white dark:bg-slate-800 w-10 h-10 flex items-center justify-center rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 shrink-0">
                      {room.roomIcon || "🎉"}
                    </span>
                  ...
                  <div className="text-xs font-bold text-slate-400 mt-1 uppercase flex flex-col gap-1.5">
                    ...
                    {room.prizeMode && (
                      <span className="text-[10px] bg-indigo-50 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400 px-2 py-0.5 rounded-md font-bold uppercase block w-fit">
                    ...
                    {room.gameMode === "four_balls_double" && (
                      <span className="text-[10px] bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-400 px-2 py-0.5 rounded-md font-bold uppercase block w-fit">
                        Estágios: {room.fourBallsStage1Limit || 30} • {room.fourBallsStage2Limit || 42} • {room.fourBallsStage3Limit || 50} bolas
                      </span>
                    )}
                    {room.onlineRadioUrl && (
                ...
                <div className="flex flex-col items-end gap-1 shrink-0">
                  ...
                  {room.prize !== undefined && (
                    <div
                      className="bg-emerald-100 dark:bg-emerald-950/55 text-emerald-700 dark:text-emerald-400 px-3 py-1 rounded-full flex items-center gap-1 font-black text-sm shadow-sm"
                      title="Prêmio"
                    >
              ...
              <div>
                ...
                <div className="space-y-3 bg-slate-50 dark:bg-slate-950/40 p-4 rounded-2xl border border-slate-200/60 dark:border-slate-800">
                  <label className="flex items-center gap-3 cursor-pointer">
                    ...
                    <div className="flex flex-col select-none">
                      <span className="font-bold text-xs text-slate-800 dark:text-slate-200">
                        Permitir Bots Participantes
                      </span>
                      <span className="text-[10px] text-slate-500 dark:text-slate-400 font-semibold">
                        Bots simulam atividade na sala
                      </span>
                    </div>
                  </label>
                  ...
                  {editBotsEnabled && (
                    <div className="pt-2.5 border-t border-slate-200/50 dark:border-slate-800">
                      <span className="block text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1.5">
                        Quantidade de Bots na Sala (Até 10)
                      </span>
                      <select
                        value={editMaxBots}
                ...
                <div className="space-y-2 bg-slate-50 dark:bg-slate-950/40 p-3 rounded-xl border border-slate-200/60 dark:border-slate-800">
                  ...
                  {editBgImage && (
                    <div className="flex items-center gap-2 pt-1 border-t border-slate-200/40 dark:border-slate-800 mt-2">
                      <div
                        className="w-16 h-8 rounded border border-slate-200 dark:border-slate-700 bg-cover bg-center overflow-hidden"
                        style={{ backgroundImage: `url(${editBgImage})` }}
                      ></div>
                      <button
                        type="button"
